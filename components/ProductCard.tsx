...
const ProductCard = ({ product }: { product: Product }) => {
  return (
    <div className="border border-gray-300 rounded-lg overflow-hidden group text-sm flex flex-col h-full">
      {/* Image + Status + Overlay */}
      <div className="border-b border-b-gray-300 relative">
        {product?.image && (
          <Link href={`/product/${product?.slug?.current}`}>
            <Image
              src={urlFor(product.image).url()}
      ...
      <div className="p-5 flex flex-col justify-between flex-grow">
        ...
        <p className="text-base text-gray-600 tracking-wide font-semibold line-clamp-1 capitalize mb-1">
          {product?.name}
        </p>
